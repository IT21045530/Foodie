...
const Post = (props) => {
  ...
  const handleOk = async () => {
    ...
    await axios
      .put(`${config.baseUrl}/post-comments/${selectedComment.id}`, data)
      .then((Value) => {
  ...
  const menu = (
    <Menu>
      ...
      <Menu.Item
        ...
        onClick={(e) => {
          axios
            .delete(`${config.baseUrl}/posts/${props.id}`)
            .then((value) => {
              window.location.reload();
            })
            .catch((err) => {
  ...
  const commentMenu = (
    <Menu>
      ...
      <Menu.Item
        ...
        onClick={(e) => {
          axios
            .delete(`${config.baseUrl}/post-comments/${item.id}`)
            .then((value) => {
              window.location.reload();
            });
        }}
  ...
  const handlePostOk = async () => {
    if (updatedPostImage) {
      ...
      const imageRef = storageRef.child(
        `posts/${localStorage.getItem("uid")}/${milliseconsds}/${
          updatedPostImage.name
        }`
      );
      ...
      await imageRef.put(updatedPostImage).then(async () => {
        imageRef.getDownloadURL().then(async (url) => {
          ...
          await axios
            .put(`${config.baseUrl}/posts/${props.id}`, d)
            .then((value) => {
              window.location.reload();
            })
            .catch((err) => {
  ...
  useEffect(() => {
    axios
      .get(`${config.baseUrl}/post-comments`)
      .then((value) => {
    ...
    axios
      .get(`${config.baseUrl}/users/${props.userName}`)
      .then((response) => {
  ...
  const handleRemoveLiked = () => {
    if (isLiked) {
      axios
        .delete(`${config.baseUrl}/like/${likeId}`)
        .then((value) => {
          setLikeCount();
          setIslLiked(false);
        })
    ...
    >
      ...
      {props.userName === localStorage.getItem("uid") && (
        ...
        >
          <div style={{ flex: 1 }}>
            {user && (
              <div className="post__username" style={{ margin: "0" }}>
      ...
      {!props.fromUser && (
        <div
          style={{ margin: "16px 8px", display: "flex", alignItems: "center" }}
        >
          {isLiked ? (
            <HeartFilled
              size={64}
      ...
      <div>
        ...
        <Button
          ...
          onClick={async (e) => {
            ...
            await axios
              .post(`${config.baseUrl}/post-comments`, data)
              .catch((err) => {
      ...
      >
        ...
        />
        <div>
          <img style={{ width: "100%" }} src={props.postImage} width="615px" />
        </div>
        <Input
          type="file"
      ...
      <Modal
        ...
        onOk={async (e) => {
          if (!rateId) {
            ...
            await axios
              .post(`${config.baseUrl}/rating`, data)
              .then((value) => {
            ...
            await axios
              .put(`${config.baseUrl}/rating/${rateId}`, data)
              .then((value) => {
